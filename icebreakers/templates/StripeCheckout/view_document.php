<?php
    ...
    $this->request->getSession()->write('title',$title);
?>
<style>		
	.btn-social-icon {
...
<div class="socialicon">						
	<a class="btn btn-social-icon btn-twitter" onclick="image('<?php echo $this->Url->build(['controller' => "/",'_full'=>true]).'//share/twitter/'.$title.'/'?>','<?php echo $image; ?>')"  style="font-size: 30px" > <span class="fa fa-twitter"></span></a>		
	<a class="btn btn-social-icon btn-linkedin" onclick="image('<?php echo $this->Url->build(['controller' => "/",'_full'=>true]).'//share/linkedin/'.$title.'/' ?>','<?php echo $image; ?>')"style="font-size: 30px"> <span class="fa fa-linkedin"></span> </a>		
	<a class="btn btn-social-icon btn-facebook" onclick="image('<?php echo $this->Url->build(['controller' => "/",'_full'=>true]).'//share/facebook/'.$title.'/' ?>','<?php echo $image; ?>')"style="font-size: 30px"> <span class="fa fa-facebook"></span> </a>		
...
<footer style="text-align:center;padding:10px;">
	<a href="<?php echo Router::url('/', true); ?>/StripeCheckout/download?title=<?php echo str_replace(array(' ','&'),array('_','@'),$title); ?>" style="color: rgb(36, 121, 216); font-size: 16px;">Download printable PDF <img style="width: 35px; margin-bottom: -5px;" src="<?php echo Router::url('/', true); ?>/images/pdf.png"></a>
</footer>
<?php
