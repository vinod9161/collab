...
print("Testing By Vinod before graph")
graph = startGraph("http://neo4j-new.collab.aws:7474/db/data/", username="neo4j", password="welcome")
print("Testing By Vinod after graph")
#event_id <- 3597
#id <- NA
#fullgraph <- NA
library(igraph)
	...
	if(!is.na(questionId)){
		...
		query1 <- paste("MATCH (n{event_id:", event_id, "})-[r:CONNECTED]->(m) WHERE NOT EXISTS(n.connection_type) RETURN ID(n) as from, ID(m) as to;")
			...
			if(!is.null(edges)){
				...
				#data$nodes$title= paste("<p><a href='https://teamsui-staging.collaboration.ai/display/",rs$user_unique_event_id[1],"/SecondDegree/",data$nodes$id,"/' target='_blank'><b>", data$nodes$label,"</b></a><br>Power Score: ",data$nodes$power_score,"<br>", data$nodes$answers,"</p>", sep="")
