...
graph = startGraph("http://localhost:7474/db/data/", username="neo4j", password="welcome")
	...
	if(fullgraph=="firstDegree"){
	    query1 <- paste("MATCH (n {event_id:", event_id, "})-[r:CONNECTED]->(m) 
			...
			if(!is.null(edges)){
				...
				if(!is.na(id) && id!='fullgraph'){
					...
					if(!is.na(fullgraph) && (fullgraph=="SecondDegree" || fullgraph=="firstDegree")){
						g <- tryCatch({
							neighbors(ig, id)
							}, error = function(err) {
								return(NULL)							
				...
				}else if(!is.na(fullgraph) && fullgraph=="firstDegree"){
				    g <- tryCatch({
				        neighbors(ig, id, mode="out")
				    }, error = function(err) {
				        return(NULL)
				    })
				}else{
